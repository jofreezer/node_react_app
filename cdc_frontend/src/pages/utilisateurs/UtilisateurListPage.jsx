...
export default function UtilisateursListPage() {
  ...
  const deleteItems = async (itemsIds) => {
    try {
      ...
      form.append('UTIL_ID', JSON.stringify(itemsIds))
      await fetchApi(`/administration/utilisateurs/delete/${itemsIds}`, {
        method: 'DELETE',
        body: form
      })
  ...
  const fetchUtilisateurs = useCallback(async () => {
    try {
      ...
      console.log(res)
      setUtilisateurs(res.result.totalRecords)
        ...
        <div className="content">
          <div className="shadow rounded mt-3 pr-1 bg-white">
            ...
            >
              ...
              <Column field="NOM" frozen header="Nom" sortable body={item => {
                return (
                  <span>{item.NOM}</span>
                )
              }} />
               <Column field="PRENOM" frozen header="Prenom" sortable body={item => {
                return (
                  <span>{item.PRENOM}</span>
                )
              }} />
              <Column field="EMAIL" frozen header="Email" sortable body={item => {
                return (
                  <span>{item.EMAIL}</span>
                )
              }} />
             
              <Column field="SEXE" frozen header="Sexe" sortable body={item => {
                return (
                  <span>{item.sexes.SEXE}</span>
                )
              }} />
               <Column field="PHONE" frozen header="Phone" sortable body={item => {
                return (
                  <span>{item.PHONE}</span>
                )
              }} />
               <Column field="DATE_NAISSANCE" frozen header="Date Naissance" sortable body={item => {
                return (
                  <span>{item.DATE_NAISSANCE}</span>
                )
              }} />
              <Column field="DESC_FONCTION" frozen header="Fonction" sortable body={item => {
                return (
                  <span>{item.fonction.DESC_FONCTION}</span>
                )
              }} />
              ...
              <Column field="" header="" alignFrozen="right" frozen body={item => {
                const items = [

                  {
                    template: () => {
                      return (
                        <span
                        onClick={() => 
                          navigate("/utilisateurs/edit",{state:inViewMenuItem })
                        }
                  ...
                  },
                  {
                    template: () => {
                      return (
                        <a href="utilisateurs" className="p-menuitem-link text-danger"
                         onClick={e => handleDeletePress(e, [inViewMenuItem.UTIL_ID])}>
